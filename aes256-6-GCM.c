...
void key_commit_attack_on_padding_fix(){
    uint8_t K0[32] = {0xcc,0x64,0x2a,0xc7,0xef,0x14,0x73,0x96,0x00,0x81,0x15,0x5d,0xd4,0x3c,0x17,0x15,0x56,0xbf,0x13,0xf9,0xc9,0x4b,0x0f,0x6f,0xa2,0x1b,0xcf,0xe2,0x00,0x13,0xe2,0x01};
    uint8_t K1[32] = {0xe8,0x72,0x2d,0xd3,0xef,0x14,0x73,0x96,0x00,0x81,0x15,0x5d,0xd4,0x85,0x17,0x15,0xef,0xed,0xc3,0x90,0xcb,0xfb,0x0b,0x6d,0x1b,0xcd,0x1f,0x8b,0x02,0x15,0xe6,0x03};
    uint8_t IV[12] = {0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x01};
    ...
    aes_256_enc(H0, K0, 6, zero);
    aes_256_enc(H1, K1, 6, zero);
    uint8_t encJ0_0[16];
    uint8_t encJ0_1[16];
    aes_256_enc(encJ0_0, K0, 6, J_0);
    aes_256_enc(encJ0_1, K1, 6, J_0);
    ...
    J_1[15] = J_0[15] + 1;
    aes_256_enc(ciphertext0, K0, 6, J_1);
    aes_256_enc(ciphertext1, K1, 6, J_1);
    ...
    uint8_t encJ1_1[32];
    J_1[15] = J_1[15] + 1;
    aes_256_enc(encJ1_0, K0, 6, J_1);
    aes_256_enc(encJ1_1, K1, 6, J_1);
    J_1[15] = J_1[15] + 1;
    aes_256_enc(encJ1_0+16, K0, 6, J_1);
    aes_256_enc(encJ1_1+16, K1, 6, J_1);
...
void key_commit_attack_verify(){
    ...
    uint8_t IV[12] = {0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x01};
    uint8_t add_data[16]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01};
    uint8_t plaintext0[48] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4f,0x61,0x8f,0xe7,0xe5,0x43,0xc4,0xb6,0x50,0x9b,0xd5,0xb8,0x2,0x14,0x87,0x66,0x83,0xf4,0x79,0x6b,0xbc,0xe7,0x45,0x2d,0x65,0x71,0x29,0xe0,0x15,0x8b,0xee,0x36};
    uint8_t plaintext1[48] = {0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x5b,0xb6,0x5,0xeb,0x9b,0xe8,0x30,0xa1,0xc2,0x9d,0x70,0xbe,0x90,0xfe,0x96,0xe8,0x42,0x3b,0x4b,0x41,0xc3,0xf8,0x68,0x6,0xc9,0xae,0x0,0x14,0x94,0xdd,0x9d,0x42};
    uint8_t ciphertext0[48] = {0};
    uint8_t ciphertext1[48] = {0};
    uint8_t tag0[16] = {0};
    uint8_t tag1[16] = {0};
    aes256gcm(ciphertext0, tag0, K0, IV, 6, plaintext0, 3, add_data, 1);
    aes256gcm(ciphertext1, tag1, K1, IV, 6, plaintext1, 3, add_data, 1);
    printf("IV\n");
    for(int i = 0; i < 12; i++){
        printf("%02x ",IV[i]);
    }printf("\n");
